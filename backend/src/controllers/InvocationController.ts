...
import { MethodKind, sanitizeRequestMetadata, type InvokeUnaryRequest, type InvokeUnaryResponse } from '@grpc-studio/shared';
...
export async function invoke(req: Request, res: Response) {
  ...
  const { target, service: serviceName, method: methodName, methodKind, data = {}, metadata } = req.body as InvokeUnaryRequest;
  ...
  const sanitizedMetadata = sanitizeRequestMetadata(metadata);
  if (!sanitizedMetadata.ok) {
    throw new AppError(sanitizedMetadata.error ?? 'Invalid request metadata', 400, 'VALIDATION_ERROR');
  }
  ...
  const result = await grpcMethodInvokerService.invokeUnary(target, serviceName, methodName, data, sanitizedMetadata.metadata);
