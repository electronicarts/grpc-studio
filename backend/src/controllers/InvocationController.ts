...
export async function invoke(req: Request, res: Response) {
  ...
  const { service: serviceName, method: methodName, methodKind, data = {} } = req.body as InvokeUnaryRequest;
  ...
  const result = await grpcMethodInvokerService.invokeUnary(serviceName, methodName, data);

  if (!result.success) {
    throw new AppError(result.error ?? 'Method invocation failed', 400, 'METHOD_ERROR');
  }

  controllerLogger.info('Method invocation successful', { service: serviceName, method: methodName });
