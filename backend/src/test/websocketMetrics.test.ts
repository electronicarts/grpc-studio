...
describe('WebSocket Metrics', () => {
  ...
  it('should track connection rejection metrics', async () => {
    ...
    // Verify metrics are still being tracked after rejection attempt
    assert.ok(metricsAfter.includes('grpc_studio_ws') || metricsBefore.includes('grpc_studio_ws'),
      'Should have WebSocket metrics registered')
  })
